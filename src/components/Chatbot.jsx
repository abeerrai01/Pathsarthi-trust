      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Messages */}
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50 scroll-smooth overscroll-contain">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                  <div className={`max-w-[85%] sm:max-w-[90%] p-3 sm:p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${
              ...
              {showChips && !isLoading && (
                <div className="grid grid-cols-2 gap-2 mt-4 pb-2">
                  {actionChips.map((chip, idx) => (
                    <button key={idx} onClick={() => handleSend(chip.query)} className="text-left p-3 text-[10px] sm:text-xs font-semibold bg-white hover:bg-indigo-50 border border-indigo-100 rounded-xl transition-all hover:border-indigo-300 text-gray-700 shadow-sm">
                      {chip.label}
                    </button>
                  ))}
                </div>
