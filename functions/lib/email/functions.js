...
const formatDetailsTable = (title, pairs) => {
    const rows = pairs
        .map((p) => `
...
exports.onNewDonation = (0, firestore_1.onDocumentCreated)({ document: "donations/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), "Donation", refId, paymentSummaryBlock);
    }
    catch (error) {
...
exports.onNewSponsor = (0, firestore_1.onDocumentCreated)({ document: "csr_partnerships/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), "Sponsor", docId, sponsorDetailsBlock);
    }
    catch (error) {
...
exports.onNewMembership = (0, firestore_1.onDocumentCreated)({ document: "memberships/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), isPremium ? "Premium Membership" : "Membership", docId, detailsBlock);
    }
    catch (error) {
...
exports.onNewJanSampark = (0, firestore_1.onDocumentCreated)({ document: "jan_sampark/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), "Jan Sampark", docId, samparkDetailsBlock);
    }
    catch (error) {
...
exports.onNewInternship = (0, firestore_1.onDocumentCreated)({ document: "internship_applications/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), "Internship", docId, internshipDetailsBlock);
    }
    catch (error) {
...
exports.onNewQuery = (0, firestore_1.onDocumentCreated)({ document: "queries/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), "Query", docId, queryDetailsBlock);
    }
    catch (error) {
...
exports.onNewVolunteer = (0, firestore_1.onDocumentCreated)({ document: "joinus_registrations/{id}", secrets: [brevo_1.brevoApiKey, brevo_1.adminEmailSecret] }, async (event) => {
    ...
    try {
        ...
        // Always notify admin
        await (0, brevo_1.sendAdminNotification)(brevo_1.brevoApiKey.value(), brevo_1.adminEmailSecret.value(), "Volunteer", docId, volunteerDetailsBlock);
    }
    catch (error) {
 ...
 */
exports.sendCustomEmail = (0, https_1.onCall)({ secrets: [brevo_1.brevoApiKey] }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in as admin.");
    }
    const data = request.data;
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", (error === null || error === void 0 ? void 0 : error.message) || "Failed to send custom email.");
    }
});
 ...
 */
exports.birthdayWishScheduler = (0, scheduler_1.onSchedule)({ schedule: "30 18 * * *", timeZone: "UTC", secrets: [brevo_1.brevoApiKey] }, async () => {
    ...
    const parseDob = (dob) => {
        if (!dob)
            return null;
        // Firestore Timestamp
        if (typeof (dob === null || dob === void 0 ? void 0 : dob.toDate) === "function") {
            const d = dob.toDate();
 ...
 */
exports.testBirthdayWish = (0, https_1.onCall)({ secrets: [brevo_1.brevoApiKey] }, async (request) => {
    ...
    if (data.testDate && typeof data.testDate === "string") {
        ...
        if (isNaN(targetDate.getTime())) {
            throw new https_1.HttpsError("invalid-argument", "testDate must be in YYYY-MM-DD format.");
        }
    }
    else {
        // Use IST "now"
    ...
    for (const docSnap of snapshot.docs) {
        ...
        if (parsed.month !== targetMonth || parsed.day !== targetDay)
            continue;
        // ✅ Birthday match!
        const name = member.fullName || member.name || "Valued Member";
        const email = member.email || "";
